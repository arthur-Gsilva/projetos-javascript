...
const fetchWeather = async (city) => {
    const APIResponse = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURI(city)}&appid=cb390709d2b91e8d299d9fab2073907f&units=metric&lang=pt_br`)
...
const renderInfo = async (city) => {
    ...
    if(data){
        ...
        `
        Máx. ${data.main.temp_max.toFixed()}°C - Min. ${data.main.temp_min.toFixed()}°C <br>
        Sábado 05/11/2022
        `
